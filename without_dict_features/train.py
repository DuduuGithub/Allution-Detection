...
from without_dict_features.bert_crf import AllusionBERTCRF
...
def train_model(model, train_dataloader, val_dataloader, optimizer, scheduler, 
    ...
    for epoch in range(num_epochs):
        # 可以根据需要动态调整权重
        if epoch <= 2:
            model.position_weight.data = torch.tensor(0.4, device=device)
        elif epoch <= 4:
...
def main():
    ...
    # 加载典故词典和类型映射
    _, type_label2id, id2type_label, num_types = load_allusion_dict()
    
    # 初始化tokenizer
    tokenizer = BertTokenizer.from_pretrained(BERT_MODEL_PATH)
    
    # 初始化模型
    model = AllusionBERTCRF(BERT_MODEL_PATH, num_types,bi_label_weight=0.99).to(device)
