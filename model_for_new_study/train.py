...
def train_model(model, train_dataloader, val_dataloader, optimizer, scheduler, 
    ...
    best_combined_score = -1  # 初始化最佳综合得分
    ...
    def calculate_combined_score(metrics, w_pos=0.6, w_type=0.4):
        """
        计算综合得分
        Args:
            metrics: 评估指标字典
            w_pos: 位置识别权重 (B/I F1的平均)
            w_type: 类别识别权重 (Top1准确率)
        """
        # 计算位置识别得分 (B和I标签的F1平均值)
        b_f1 = metrics["position"]["B"]["f1"]
        i_f1 = metrics["position"]["I"]["f1"]
        position_score = (b_f1 + i_f1) / 2
        
        # 获取类别识别得分 (Top1准确率)
        type_score = metrics["type"]["top1_acc"]
        
        # 计算综合得分
        combined_score = w_pos * position_score + w_type * type_score
        
        return combined_score, position_score, type_score
    ...
    for epoch in range(num_epochs):
        ...
        # 计算综合得分
        combined_score, position_score, type_score = calculate_combined_score(metrics)
        
        log_message(f"\nValidation Scores:")
        log_message(f"Position Score (B/I F1): {position_score:.4f}")
        log_message(f"Type Score (Top1 Acc): {type_score:.4f}")
        ...
        # 2. 位置识别和类别识别都达到最低阈值
        # 3. 训练轮次超过最小轮次
        if (combined_score > best_combined_score and 
            position_score >= 0.3 and  # 位置识别最低要求
            type_score >= 0.2):             # 类别识别最低要求
            ...
            torch.save({
                ...
                f'best_model_e{epoch}_p{position_score:.3f}_t{type_score:.3f}.pt'))
            
            log_message(f'Saved new best model:')
            log_message(f'- Combined Score: {combined_score:.4f}')
            log_message(f'- Position Score: {position_score:.4f}')
            log_message(f'- Type Score: {type_score:.4f}')
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            log_message(f'Best Combined Score: {best_combined_score:.4f}')
            break
...
def main():
    ...
    model = AllusionBERTCRF(BERT_MODEL_PATH, num_types, dict_size,
                            bi_label_weight=0.15,position_weight=1.0).to(device)
    ...
    val_dataset = PoetryNERDataset(
        os.path.join(DATA_DIR, 'valid_data.csv'),
        tokenizer, MAX_SEQ_LEN,
        type_label2id=type_label2id,
        id2type_label=id2type_label,
        features_path=features_path,
