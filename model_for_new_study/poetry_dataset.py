...
class PoetryNERDataset(Dataset):
    ...
    def __init__(self, file_path, tokenizer, max_len, type_label2id, id2type_label, 
                 features_path=None, mapping_path=None, negative_sample_ratio=0.01):
        ...
        try:
            ...
            if variation_number > 0:
                ...
                if len(parts)>=8:
                    ...
                    transformed_allusions = parts[4].strip().split(';')
    ...
    def __getitem__(self, idx):
        ...
        if self.precomputed_features is not None:
            try:
                ...
                # 尝试查找可能带有额外空格的相同文本
                for key in self.sentence_to_id:
                    if text.replace(' ', '') == key.replace(' ', ''):  # 移除所有空格后比较
                        sent_id = self.sentence_to_id[key]
                        print(f"Found match with different spacing. Original: '{text}' -> Matched: '{key}'")
                        break
                else:  # 如果还是没找到匹配
