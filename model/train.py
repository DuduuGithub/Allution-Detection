...
def evaluate_metrics_from_outputs(outputs, labels):
    ...
    # 添加负例统计
    negative_total = 0      # 总负例数
    negative_correct = 0    # 正确预测的负例数
    ...
    for batch_outputs, batch_labels in zip(outputs, labels):
        ...
        # 确保预测结果和标签数量匹配
        for pred_info in type_predictions:
            start, end, pred_types = pred_info  # 解包预测结果
            # 获取预测的类型（top1-5）
            pred_type_ids = [p[0] for p in pred_types]
            
            # 在type_labels中找到对应位置的标签
            for batch_idx in range(batch_size):
                for type_idx in range(max_type_len):
                    true_type = type_labels[batch_idx][type_idx].item()
                    
                    if true_type > 0:  # 有效的典故类型标签
                        type_true_total += 1
                        type_pred_total += 1
                        
                        # 检查top1准确率
                        if pred_type_ids[0] == true_type:
                            type_correct += 1
                            
                        # 检查top3准确率
                        if true_type in pred_type_ids[:3]:
                            type_top3_correct += 1
                            
                        # 检查top5准确率
                        if true_type in pred_type_ids[:5]:
                            type_top5_correct += 1
                    else:  # 负样本
                        negative_total += 1
    ...
    metrics = {
        ...
        'type': {
            'precision': type_correct / type_pred_total if type_pred_total > 0 else 0,
            'recall': type_correct / type_true_total if type_true_total > 0 else 0,
            'f1': 2 * type_correct / (type_pred_total + type_true_total) if (type_pred_total + type_true_total) > 0 else 0,
            'top3_acc': type_top3_correct / type_true_total if type_true_total > 0 else 0,
            'top5_acc': type_top5_correct / type_true_total if type_true_total > 0 else 0,
            'negative_samples': {
                'total': negative_total,
                'correct': negative_correct,
                'accuracy': negative_correct / negative_total if negative_total > 0 else 0
            }
        }
    }
...
def train_model(model, train_dataloader, val_dataloader, optimizer, scheduler, 
                device, num_epochs, save_dir, position_weight=0.6, bi_label_weight=0.6, 
                id2type_label=None):
    """
    联合训练模型
    Args:
    ...
    # 设置B/I标签的权重
    model.bi_label_weight.data = torch.tensor(bi_label_weight, device=device)
    
    for epoch in range(num_epochs):
        # 可以根据需要动态调整权重
        if epoch <= 2:
            model.position_weight.data = torch.tensor(0.8, device=device)
        else:
        ...
        with torch.no_grad():
            for batch in val_dataloader:
                ...
                # print('type_pred_outputs:',type_pred_outputs)
                # 收集预测结果和标签用于评估
        ...
        if avg_val_loss < best_val_loss:
            ...
            log_message(f'Saved new best model with validation loss: {best_val_loss:.4f}')
        ...
        log_message(f'  Precision: {metrics["type"]["precision"]:.4f}')
        log_message(f'  Recall: {metrics["type"]["recall"]:.4f}')
        log_message(f'  F1: {metrics["type"]["f1"]:.4f}')
        log_message(f'  Top-3 Accuracy: {metrics["type"]["top3_acc"]:.4f}')
        log_message(f'  Top-5 Accuracy: {metrics["type"]["top5_acc"]:.4f}')
        log_message(f'  Negative Samples:')
        log_message(f'    Total: {metrics["type"]["negative_samples"]["total"]}')
        log_message(f'    Correct: {metrics["type"]["negative_samples"]["correct"]}')
        log_message(f'    Accuracy: {metrics["type"]["negative_samples"]["accuracy"]:.4f}')
        log_message('='*50)
...
def main():
    ...
    train_model(
        ...
        bi_label_weight=0.6,  # B/I标签相对于O标签的权重
        id2type_label=id2type_label
    )
