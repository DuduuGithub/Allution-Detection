...
from without_dict_features.poetry_dataset import PoetryNERDataset
from without_dict_features.test_bert import AllusionBERTCRF
from without_dict_features.config import (
...
def calculate_weighted_bio_score(metrics, w_b=0.4, w_i=0.4, w_o=0.2):
    """
    计算BIO标签的加权F-score
    Args:
        metrics: 评估指标字典
    ...
    Returns:
        weighted_score: 加权分数
    """
    b_f1 = metrics["position"]["B"]["f1"]
    i_f1 = metrics["position"]["I"]["f1"]
    o_f1 = metrics["position"]["O"]["f1"]
    
    weighted_score = w_b * b_f1 + w_i * i_f1 + w_o * o_f1
    return weighted_score
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(val_dataloader):
                ...
                # # 对每个batch中的样本进行详细输出
                # for i in range(min(2, len(batch['text']))):
                #     print(f"\nBatch {batch_idx}, 样本 {i}:")
                #     text = batch['text'][i]
                #     print(f"文本: {text}")
                    
                #     # 输出位置标签和预测
                #     true_positions = position_labels[i][1:len(text)+1].tolist()  # 去掉CLS和SEP
                #     pred_positions = position_pred_outputs['position_predictions'][i]
                ...
                #     print("\n位置标注对比:")
                #     print("真实标签:", end=" ")
                #     for j, label in enumerate(true_positions):
                #         if label == 1:
                #             print(f"\033[91mB\033[0m", end=" ")  # 红色显示B
                #         elif label == 2:
                #             print(f"\033[92mI\033[0m", end=" ")  # 绿色显示I
                #         else:
                #             print("O", end=" ")
                    
                #     print("\n预测标签:", end=" ")
                #     for j, label in enumerate(pred_positions):
                #         if label == 1:
                #             print(f"\033[91mB\033[0m", end=" ")  # 红色显示B
                #         elif label == 2:
                #             print(f"\033[92mI\033[0m", end=" ")  # 绿色显示I
                ...
                #     # 输出类型标签和预测（考虑多个典故）
                #     print("\n\n类型识别对比:")
                #     sample_target_positions = target_positions[i]  # [max_allusions, 2]
                #     sample_type_labels = type_labels[i]  # [max_allusions]
                    
                #     # 获取当前样本的所有典故预测
                #     type_preds = type_pred_outputs['type_predictions']
                    
                #     # 遍历每个可能的典故位置
                #     for allusion_idx in range(len(sample_type_labels)):
                #         # 检查这个位置是否有真实的典故
                #         if sample_target_positions[allusion_idx].sum() > 0:
                #             print(f"\n典故 {allusion_idx + 1}:")
                #             # 打印真实标签
                #             true_type = sample_type_labels[allusion_idx].item()
                #             print(f"真实类型: {id2type_label[true_type] if true_type in id2type_label else true_type}")
                #             print(f"真实位置: [{sample_target_positions[allusion_idx][0].item()}, {sample_target_positions[allusion_idx][1].item()}]")
                            
                #             # 查找对应的预测结果
                #             found_pred = False
                #             for start, end, pred_types in type_preds:
                #                 # 检查位置是否匹配（考虑CLS token的偏移）
                #                 if (start + 1 == sample_target_positions[allusion_idx][0].item() and 
                #                     end + 1 == sample_target_positions[allusion_idx][1].item()):
                #                     print("预测类型 (Top-5):")
                #                     for type_id, score in pred_types:
                #                         type_name = id2type_label[type_id] if type_id in id2type_label else type_id
        ...
        # 计算加权BIO分数
        weighted_bio_score = calculate_weighted_bio_score(metrics)

        # # 保存最佳模型（基于加权BIO分数）
        # if weighted_bio_score > best_weighted_score:
        #     best_weighted_score = weighted_bio_score
        #     torch.save({
        #         'epoch': epoch,
        #         'model_state_dict': model.state_dict(),
        #         'optimizer_state_dict': optimizer.state_dict(),
        #         'loss': avg_val_loss,
        #         'weighted_bio_score': weighted_bio_score,
        #     }, os.path.join(save_dir, 'best_model.pt'))
        #     log_message(f'Saved new best model with weighted BIO score: {weighted_bio_score:.4f}')
        #     patience_counter = 0  # 重置耐心计数器
        # else:
        #     patience_counter += 1  # 增加耐心计数器

        # 保存最佳模型（基于类别识别的top1准确率）
        current_top1_acc = metrics["type"]["top1_acc"]
        if current_top1_acc > best_weighted_score:  # 复用变量名但现在存储最佳top1准确率
            best_weighted_score = current_top1_acc
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
            ...
            log_message(f'Saved new best model with top1 accuracy: {current_top1_acc:.4f}')
            patience_counter = 0  # 重置耐心计数器
        else:
            patience_counter += 1  # 增加耐心计数器
...
def main():
    ...
    model = AllusionBERTCRF(BERT_MODEL_PATH, num_types,
